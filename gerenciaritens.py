...
def mostrarItens(itens):
    return Div(H2("Itens"),Table(
        ...
        Tbody(
            *[Tr(Td(item.nome), Td(item.quantidade), Td(item.status), Td(item.motivo) ,  Td(Form(hx_get=f"/removerItem/{item.codigo}", hx_swap="InnerHTML",hx_target='#tabelaitens')
            (Fieldset(Button('Apagar', type='submit')))
            )) for item in itens]
        ),
...
def formAdicionarItens(codigoobs):
    ...
    return Div(Div(H3("Adicionar Item"),Form(method="post", hx_post=f"/adicionaritem", hx_target='#tabelaitens', hx_swap="InnerHtml")
    ...
    Input(name="codigoobs",value=f"{obs.codigo}", style="display:none"), Button("Adicionar", type="submit", style="grid-column: 1 / span 2; grid-row: 3 ;place-self: center"),
     style="display: grid; grid-template-columns: repeat(2fr, 2fr); gap: 5px; align-items: center;")
     ), style="""
                display: flex-inline;
                ...
                margin: 0 auto;
                text-align: center;
            """),  Div(mostrarItens(itens_lista), style="""
                display: flex-inline;
                justify-content: center;  
                align-items: center;
                width: 40%;
